...
export async function crearSesion(req, res) {
  try {
    ...
    if (!titulo || !clienteId) return res.status(400).json({ error: "Faltan campos obligatorios (titulo, clienteId)" });
...
export async function actualizarSesion(req, res) {
  try {
    ...
    sesiones[idx] = {
      ...
      ejerciciosAsignados: Array.isArray(ejerciciosAsignados) ? ejerciciosAsignados : sesiones[idx].ejerciciosAsignados,
